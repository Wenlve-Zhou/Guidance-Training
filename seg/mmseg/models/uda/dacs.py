...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        if not self.source_only:
            ...
            mix_losses, debug1, debug2 = self.get_model().forward_train(
                mixed_img, img_metas, mixed_lbl, seg_weight=mixed_seg_weight, return_feat=False,
                pseudo_label=pseudo_label.unsqueeze(1),
                pseudo_weight=pseudo_weight,
                mask=1 - torch.cat(mix_masks, dim=0),
                return_debug=True)
            seg_debug['Mix'] = debug1
            seg_debug['Mix++'] = debug2
