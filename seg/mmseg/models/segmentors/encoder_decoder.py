...
class EncoderDecoder(BaseSegmentor):
    ...
    def guidance_training(self, x, img_metas, pseudo_label, pseudo_weight, mask, return_logits=False):
        losses = dict()
        transform_feature = self.guider(x, mask)
        pseudo_weight = Guider.calibrated_pseudo_weight(pseudo_weight, mask)
        loss_decode = self.decode_head.forward_train(transform_feature, img_metas,
                                                     pseudo_label,
                                                     self.train_cfg,
                                                     pseudo_weight,return_logits)

        losses.update(add_prefix(loss_decode, 'guidance'))
        return losses
    ...
    def forward_train(self,
        ...
        if pseudo_label is not None:
            loss_cp = self.guidance_training(x, img_metas,
                                             pseudo_label,
                                             pseudo_weight,
                                             mask,
                                             return_logits)
